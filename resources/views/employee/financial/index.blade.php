                        ...
                        <tbody>
                            @foreach ($financialRecords as $record)
                                <tr data-id="{{ $record->id }}" data-platform-id="{{ $record->platform_id }}"
                                    ...
                                    data-note="{{ htmlspecialchars($record->note) }}"
                                    ...
                                    <td>
                                        @if ($record->revenue_sources)
                                            @foreach ($record->revenue_sources as $src)
                                                <div>{{ $src['source_name'] }}: {{ number_format($src['amount'], 2) }}
                                                </div>
                                            @endforeach
                                        @else
                                            <div>N/A</div>
                                    ...
                                    <td>
                                        @foreach ($record->expenses as $exp)
                                            <div>{{ $exp->description ?? 'N/A' }} ({{ $exp->expenseType->name }}):
                                                {{ number_format($exp->amount, 2) }}</div>
                                        @endforeach
                        ...
                        <div id="expense-container">
                            <div class="expense-row mb-3 p-3 border rounded">
                                <div class="row">
                                    ...
                                    <div class="col-md-3 mb-2">
                                        <label class="form-label">Tên nguồn</label>
                                        <input type="text" name="expenses[0][source_name]" class="form-control"
                                            required>
                                    </div>
                                    <div class="col-md-3 mb-2">
                                        <label class="form-label">Số tiền</label>
...
@section('scripts')
    <script>
        ...
        let editRevenueSourceIndex = 0;

        function addRevenueSourceRow(containerId = 'revenue-source-container') {
            const container = document.getElementById(containerId);
            const index = containerId === 'revenue-source-container' ? revenueSourceIndex++ : editRevenueSourceIndex++;
        ...
        document.querySelectorAll('.edit-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Parse note to get original note and revenue sources
                let noteData = {};
                try {
                    noteData = JSON.parse(row.getAttribute('data-note') || '{}');
                } catch (e) {
                    noteData = {};
                }
                form.querySelector('textarea[name="note"]').value = noteData.note || '';

                // Populate revenue sources
                const revenueSourceContainer = document.getElementById('edit-revenue-source-container');
                revenueSourceContainer.innerHTML = '';
                editRevenueSourceIndex = 0;
                const revenueSources = noteData.revenue_sources || [];
                revenueSources.forEach((source, index) => {
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 mb-2">
                            <label class="form-label">Số tiền</label>
                            <input type="number" name="revenue_sources[${index}][amount]" class="form-control" step="0.01" value="${source.amount || ''}" required>
                        </div>
                        <div class="col-md-1 mb-2 d-flex align-items-end">
                            <button type="button" class="btn btn-danger btn-sm" onclick="this.closest('.revenue-source-row').remove()">Xóa</button>
                        </div>
                ...
                `;
                    ...
                    editRevenueSourceIndex = index + 1;
                });
                ...
                expenses.forEach((expense, index) => {
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-2">
                            <label class="form-label">Tên nguồn</label>
                            <input type="text" name="expenses[${index}][source_name]" class="form-control" value="${expense.description || ''}" required>
                        </div>
                        <div class="col-md-3 mb-2">
                            <label class="form-label">Số tiền</label>
