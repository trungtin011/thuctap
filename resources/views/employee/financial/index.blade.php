        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="financialForm" action="{{ route('employee.financial.store') }}" method="POST">
                        ...
                        <div class="expense-row mb-3 p-3 border rounded">
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-2">
                                    ...
                                    <input type="number" name="expenses[0][amount]" class="form-control" step="0.01" required>
                    ...
                    <button type="button" class="btn btn-outline-secondary mb-3" onclick="addExpenseRow('expense-container')">Thêm chi phí</button>
...
<script>
    ...
    document.querySelectorAll('.edit-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            form.querySelector('select[name="platform_id"]').value = row.getAttribute('data-platform-id');
            form.querySelector('input[name="revenue"]').value = row.getAttribute('data-revenue');
            form.querySelector('input[name="record_date"]').value = row.getAttribute('data-record-date');
            form.querySelector('input[name="record_time"]').value = row.getAttribute('data-record-time');
    ...
    document.getElementById('editFinancialForm').addEventListener('submit', function (e) {
        ...
        .then(data => {
            if (data.success) {
                const modal = bootstrap.Modal.getInstance(document.getElementById('editFinancialModal'));
                modal.hide();
                Swal.fire({
                    icon: 'success',
                    title: 'Thành công!',
