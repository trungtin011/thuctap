    ...
    <div class="mx-auto p-6">
        ...
        <form method="POST" action="{{ route('admin.targets.store') }}" class="bg-white p-6 rounded shadow">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="target_amount" id="target_amount" class="form-control" required min="0"
                    step="1000" placeholder="VD: 1000000000" value="{{ old('target_amount') }}">
            </div>
            <div class="mb-4">
                        ...
                        @foreach ($targets as $target)
                            @php
                                ...
                                $percent =
                                    $target->target_amount > 0
                                        ? round(($totalRevenue / $target->target_amount) * 100, 2)
                                        : 0;
                            ...
                            <tr>
                                ...
                                <td class="py-2 px-2 border-b">
                                    {{ $target->department ? $target->department->name : 'Toàn công ty' }}</td>
                                <td class="py-2 px-2 border-b">{{ number_format($target->target_amount, 0, ',', '.') }} VNĐ
                                </td>
                                <td class="py-2 px-2 border-b">{{ number_format($totalRevenue, 0, ',', '.') }} VNĐ</td>
                                <td class="py-2 px-2 border-b">{{ $percent }}%</td>
                                <td class="py-2 px-2 border-b text-red-600">{{ number_format($remain, 0, ',', '.') }} VNĐ
                                </td>
                                <td class="py-2 px-2 border-b">
